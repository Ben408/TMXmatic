...
class Tmx(TmxElement):
    ...
    def to_tmx(self, file: str | bytes | PathLike, encoding: str = "utf-8") -> None:
        ...
        with open(file, "wb") as f:
            tree.write(f, xml_declaration=True, encoding=encoding)

    def to_csv(self, file: str | bytes | PathLike, encoding: str = "utf-8") -> None:
        with open(file, "wb", newline="") as f:
            csv_writer = writer(f)
            for tu in self:
                csv_writer.writerow(
                    [
                        tostring(tuv.segment.to_element(), encoding=encoding)
                        .decode()
                        .lstrip(5)
                        .rstrip(6)
                        for tuv in tu.tuvs
